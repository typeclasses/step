...
castTo :: forall act2 act1 xs x r s e m a. (Monad m, Is act1 act2) => act1 xs x r s e m a -> act2 xs x r s e m a
...
instance Is SureQuery Any where
    cast (SureQuery x) = castTo @Any (mapError absurd x)

-- | Everything lifts to Any
instance Is Atom Any where
    cast (Atom q) = Any_Join (castTo @Any (fmap (castTo @Any) q))
