...
class
    ...
    IsStep (step :: Action)
  where
    contramapStep :: Functor m => (r' -> r) -> step xs x r s e m a -> step xs x r' s e m a
...
class
    ( forall xs x r s e m. Functor m =>
          Functor (walk xs x r s e m)
    ) =>
    IsWalk (walk :: Action)
  where
    contramapWalk :: Functor m => (r' -> r) -> walk xs x r s e m a -> walk xs x r' s e m a
