...
loadingCursor = Cursor{ Cursor.init, Cursor.input, Cursor.commit, Cursor.extract }
  where
    ...
    input, bufferedInput, freshInput :: Stream (RST (Stream m xs x) (DoubleBuffer xs x) m) xs x
    ...
    commit, commitBuffered, commitFresh :: Positive Natural -> RST (Stream m xs x) (DoubleBuffer xs x) m AdvanceResult
