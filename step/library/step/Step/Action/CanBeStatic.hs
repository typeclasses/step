...
instance CanBeStatic Query     where trivial x = Query     \_ -> StateT \s -> return (Right x, s)
...
instance CanBeStatic SureQuery where trivial x = SureQuery \_ -> StateT \s -> return (x, s)
