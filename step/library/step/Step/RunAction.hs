...
runQuery CursorRunR{ inputRunR, runR, resetRunR } = runR . r
  where
    r :: forall a'. Query xs x r s m a' -> RST r (CursorState s s') m (Either r a')
    r = \case
...
runAny :: forall xs x r s s' m a. Monad m => CursorRunRW xs x r s s' m -> Any xs x r s m a -> RST r s m (Either r a)
runAny CursorRunRW{ inputRunRW, runRW, commitRunRW, resetRunRW } = runRW . r
  where
    r :: forall a'. Any xs x r s m a' -> RST r (CursorState s s') m (Either r a')
    r = \case
