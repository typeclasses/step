...
takeChar :: forall c m r. Chunk c => Atom c m r r (One c)
...
takeParticularChar :: forall c m r. Chunk c => Eq (One c) => One c -> Atom c m r r ()
takeParticularChar c = P.do{ nextCharIs c P.>>= requireTrue; trySkipChar }

satisfyJust :: forall c m r a. Chunk c => (One c -> Maybe a) -> Atom c m r r a
satisfyJust ok = P.do{ x <- peekChar; y <- requireJust (ok x); trySkipChar; P.pure y }

satisfyPredicate :: forall c m r. Chunk c => (One c -> Bool) -> Atom c m r r (One c)
